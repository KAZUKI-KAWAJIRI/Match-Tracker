...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const verifySession = async () => {
      try {
        console.log('AuthGuard: セッション検証開始', { isAuthPage, isLoading, isAuthenticated })
        ...
        if (!isLoading) {
          ...
          if (!isAuthenticated && !isAuthPage) {
            console.log('AuthGuard: 未認証状態を検出、ログインページへリダイレクト')
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT' && !isAuthPage) {
        console.log('AuthGuard: サインアウト検出、ログインページへリダイレクト')
        if (isMounted) {
  ...
  }, [isAuthPage, router, isAuthenticated, isLoading, supabase, refreshSessionStatus])
